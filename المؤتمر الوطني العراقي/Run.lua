...
EVERTON = Token:match("(%d+)")
redis:set(EVERTON..":VERSION",1)
redis:set(EVERTON..":SUDO_ID:",SUDO_USER)
redis:set(EVERTON..":DataCenter:",'German')
redis:set(EVERTON..":UserNameBot:",BOT_User)
redis:set(EVERTON..":NameBot:",BOT_NAME)
redis:hset(EVERTON..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3EVERTON_INSTALL","Yes")
info = {}
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس إيفرتون بنسبه 100% ]  }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE EVERTON source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JOKER : @eljo_ker_m  }•\e[0m"

echo -e "\e[38;5;77m      EVERTON ✾ @eljo_ker_me[0m"
...
EVERTON = Token:match("(%d+)")
our_id = tonumber(EVERTON)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(EVERTON..":UserNameBot:")
SUDO_ID = tonumber(redis:get(EVERTON..":SUDO_ID:"))
SUDO_USER = redis:hgetall(EVERTON..'username:'..SUDO_ID).username
version = redis:get(EVERTON..":VERSION")
DataCenter = redis:get(EVERTON..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..EVERTON..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(EVERTON..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(EVERTON..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(EVERTON..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(EVERTON..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(EVERTON..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(EVERTON..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(EVERTON..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(EVERTON..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(EVERTON..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(EVERTON..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(EVERTON..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(EVERTON..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(EVERTON..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(EVERTON..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(EVERTON..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس إيفرتون ⇐ @UP_II. مـعرف المطور  :]]..SUDO_USER:gsub([[\_]],'_')..[[ .
]])
return false
end
if not msg then
...
if msg.GroupActive and redis:get(EVERTON..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(EVERTON..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(EVERTON..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(EVERTON..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.EVERTON then 
if msg.text and ScriptFile.iEVERTON then
for k, EVERTON in pairs(ScriptFile.EVERTON) do
local SearchText = msg.text:match(EVERTON)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iEVERTON(msg,{msg.text:match(EVERTON)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",EVERTON.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dEVERTON then
if ScriptFile.dEVERTON(msg) == false then
return false
end
...
if Plug.EVERTON then 
if msg.text and Plug.iEVERTON then
for k, EVERTON in pairs(Plug.EVERTON) do
local SearchText = msg.text:match(EVERTON)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(EVERTON..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(EVERTON..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iEVERTON(msg,{msg.text:match(EVERTON)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",EVERTON.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dEVERTON then
Plug.dEVERTON(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source EVERTON")
end 

end
else
print("The File Script.lua Not Runing in The Source EVERTON")
...
function tdcli_update_callback(data)
	...
	redis:srem(EVERTON..'users',msg.sender_user_id_)
	end
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(EVERTON..":SUDO_ID:")
redis:hset(EVERTON..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(EVERTON..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(EVERTON..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(EVERTON..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(EVERTON..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(EVERTON..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(EVERTON..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(EVERTON..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(EVERTON..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(EVERTON..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(EVERTON..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(EVERTON..'group:add'..msg.chat_id_) then
	redis:set(EVERTON..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(EVERTON..'group:add'..data.chat_.id_) then
	redis:set(EVERTON..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(EVERTON..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(EVERTON..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(EVERTON..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	EVERTON..'group:name'..k,IDS.Title,
	EVERTON..'num_msg_EVERTON'..k,5,
	EVERTON..'group:add'..k,true,
	EVERTON..'lock_link'..k,true,
	EVERTON..'lock_spam'..k,true,
	EVERTON..'lock_webpage'..k,true,
	EVERTON..'lock_markdown'..k,true,
	EVERTON..'lock_flood'..k,true,
	EVERTON..'lock_bots'..k,true,
	EVERTON..'mute_forward'..k,true,
	EVERTON..'mute_contact'..k,true,
	EVERTON..'mute_document'..k,true,
	EVERTON..'mute_inline'..k,true,
	EVERTON..'lock_username'..k,true,
	EVERTON..'replay'..k,true
	)
	redis:sadd(EVERTON..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(EVERTON..'username:'..ID,'username',user)
	redis:sadd(EVERTON..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(EVERTON..'username:'..ID,'username',user)
	redis:sadd(EVERTON..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(EVERTON..'username:'..ID,'username',user)
	redis:sadd(EVERTON..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(EVERTON..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(EVERTON..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(EVERTON..'username:'..data.user_.id_).username
	...
	redis:hset(EVERTON..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(EVERTON..'group:ids')
	...
	local pv = redis:smembers(EVERTON..'users')
	local NumPvDel = 0
	for i = 1, #pv do
